...
APPS_OUTPUT=$(az webapp list --resource-group "$RESOURCE_GROUP" --query "[?contains(name, 'app-fs-judgepapers')].{name:name, rg:resourceGroup}" -o tsv)

# Check if emptiness
if [ -z "$APPS_OUTPUT" ]; then
    echo "Error: Could not find any Web App matching 'app-fs-judgepapers'. Make sure you are logged into Azure CLI."
    exit 1
fi
...
FUNC_INFO=$(az functionapp list --resource-group "$RESOURCE_GROUP" --query "[?contains(name, 'func-fs-judgepapers')].{name:name, host:defaultHostName}" -o tsv | head -n 1)
