...
**The Function App itself must be `AllowAnonymous`** (`function.bicep` `globalValidation`), *not* `requireAuthentication: true` — the proxy forwards only the email header, no bearer token, so Easy Auth enforcement would 401 every proxied request (`WWW-Authenticate: Bearer`, empty body) before the app's header check runs. NB: inbound IP restrictions (locking the function to the Web App's IPs) are **not viable** — they `403` the GitHub runner during the Flex Consumption deploy's sync-triggers/health-check step and hang the pipeline. The function endpoint is therefore public; identity is enforced only by the app's forwarded-header check (a known trust assumption — a shared-secret header between `server.js` and the function would harden it).
